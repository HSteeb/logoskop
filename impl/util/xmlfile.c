...
Contributor(s):
2000-01-20 Helmut Steeb (reportError does nothing)
2000-03-03 Helmut Steeb (avoid exit in low memory situation)
...
static int sErrCode = 0;
static int sErrLine = 0;
static int sErrCol  = 0;
extern int XML_GetStoredErrCode() { return sErrCode; };
extern int XML_GetStoredErrLine() { return sErrLine; };
extern int XML_GetStoredErrCol()  { return sErrCol; };

static
void reportError(XML_Parser parser, const XML_Char *filename)
{
/* HS 2000-01-20 changed
...
*/
  ...
  sErrCode = XML_GetErrorCode(parser);
  message = XML_ErrorString(sErrCode);
  filename;
  if (message)
  {
    sErrLine = XML_GetErrorLineNumber(parser);
    sErrCol  = XML_GetErrorColumnNumber(parser);
  }
  else {
    sErrLine = -1; // unknown
    sErrCol  = -1;
		    ...
		    unsigned flags)
{
  ...
  if (!XML_SetBase(parser, filename)) {
    ...
    // HS 2000-03-03: avoid exit in low memory situation
    // exit(1);
    return 0;
  }
