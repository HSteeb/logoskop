...
/* HS 000825 simulate GList code which I did not even find on my Linux system... */
		       ...
		       gint *pmax_level)
{
  ...
  if (fribidi_debug)
      printf("Resolving neutral types.\n");
  ...
  if (fribidi_debug)
    printf("Resolving implicit levels.\n");
  {
  ...
  *pbase_dir = base_dir;
  printf("fribidi_analyse_string done.\n");
}
		     ...
		     )
{
  ...
  if (len > (2L<<16)-1)
    {
      printf("Fribidi can't handle strings > 65000 chars!\n");
      return;
    }
  ...
  /* Free up the rl_list */
#ifdef _WIN32
  /* HS 000825: to avoid simulating g_list_foreach, I simply replace the orginal code...*/
  g_list_free_deep(type_rl_list);
#else
